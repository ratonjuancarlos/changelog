...
const BRANCH_PREFIX = new RegExp(`^(${BRANCH_SEARCH}|${PROJECT_SEARCH}|${BRANCH_PREFIX_SIN}|\\d+)(-|\\u005c|\\u002F)+`, 'gi');

// const BRANCH_PREFIXAng = new RegExp(`^(ci)(\\(\\))(: cz conventional changelog)`, 'gi');
...
function commonCommit(commit) {
    ...
    let branchType;
// aca varia el branch prefix
    branchType = commit.subjectClean.match(BRANCH_PREFIX);
